...
const Onidex = artifacts.require("./Onidex.sol");

require("chai")
  .use(require("chai-as-promised"))
  .should();

contract("Onidex", async (accounts) => {
  let onidex, result, cryptoBoyCount;

  before(async () => {
    onidex = await Onidex.deployed();
  });

  describe("Deployment", async () => {
    it("contract has an address", async () => {
      const address = await onidex.address;
    ...
    it("has a name", async () => {
      const name = await onidex.collectionName();
      assert.equal(name, "Onidex Collection");
    });

    it("has a symbol", async () => {
      const symbol = await onidex.collectionNameSymbol();
      assert.equal(symbol, "ONI");
    });
  });

  describe("application features", async () => {
    it("allows users to mint ERC721 token", async () => {
      cryptoBoyCount = await onidex.cryptoBoyCounter();
      assert.equal(cryptoBoyCount.toNumber(), 0);

      let tokenExists;
      tokenExists = await onidex.getTokenExists(1, { from: accounts[0] });
      assert.equal(tokenExists, false);

      let tokenURIExists;
      tokenURIExists = await onidex.tokenURIExists(
      ...
      let tokenNameExists;
      tokenNameExists = await onidex.tokenNameExists("myCBNFT", {
        from: accounts[0],
      });
      ...
      for (let i = 0; i < colorsArray1.length; i++) {
        colorExists = await onidex.colorExists(colorsArray1[i], {
          from: accounts[0],
        });
        assert.equal(colorExists, false);
      }

      result = await onidex.mintCryptoBoy(
        "myCBNFT",
        "https://gateway.pinata.cloud/ipfs/QmYFmJgQGH4uPHRYN15Xdv4aLd9o4Aq63y1e4GgN6kj5aK/2",
      ...
      let cryptoboy;
      cryptoboy = await onidex.allNFTs(1, {
        from: accounts[0],
      });
    ...
    it("returns total number of tokens minted so far", async () => {
      const totalNumberOfTokensMinted = await onidex.getNumberOfTokensMinted();
      assert.equal(totalNumberOfTokensMinted.toNumber(), 4);
    });

    it("returns total number of tokens owned by an address", async () => {
      const totalNumberOfTokensOwnedByAnAddress = await onidex.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
    ...
    it("allows users to buy token for specified ethers", async () => {
      const oldTokenOwner = await onidex.getTokenOwner(1);
      ...
      oldTotalNumberOfTokensOwnedBySeller = await onidex.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
      ...
      result = await onidex.buyToken(1, {
        from: accounts[2],
        value: web3.utils.toWei("1", "Ether"),
      });

      const newTokenOwner = await onidex.getTokenOwner(1);
      ...
      newTotalNumberOfTokensOwnedBySeller = await onidex.getTotalNumberOfTokensOwnedByAnAddress(
        accounts[0]
      );
      ...
      await onidex.buyToken(2, {
        from: 0x0000000000000000000000000000000000000000,
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;

      await onidex.buyToken(56, {
        from: accounts[4],
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;

      await onidex.buyToken(3, {
        from: accounts[0],
        value: web3.utils.toWei("1", "Ether"),
      }).should.be.rejected;
    });
    ...
    it("allows users to change token price", async () => {
      let cryptoBoyPrice;
      cryptoBoyPrice = await onidex.allNFTs(1, {
        from: accounts[0],
      });
      assert.equal(web3.utils.fromWei(cryptoBoyPrice.price, "ether"), 1);

      result = await onidex.changeTokenPrice(
        1,
      ...
      await onidex.changeTokenPrice(1, web3.utils.toWei("3", "Ether"), {
        from: 0x0000000000000000000000000000000000000000,
      }).should.be.rejected;

      await onidex.changeTokenPrice(82, web3.utils.toWei("3", "Ether"), {
        from: accounts[2],
      }).should.be.rejected;
    ...
    it("allows users to toggle between setting the token for sale or not for sale", async () => {
      ...
      result = await onidex.toggleForSale(1, { from: accounts[2] });
      ...
      await onidex.toggleForSale(1, {
        from: 0x0000000000000000000000000000000000000000,
      }).should.be.rejected;

      await onidex.toggleForSale(94, { from: accounts[2] }).should.be
        .rejected;

      await onidex.toggleForSale(1, { from: accounts[8] }).should.be
        .rejected;
    });
  });
});
